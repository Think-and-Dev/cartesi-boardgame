pub mod server {
    ...
    pub async fn send_finish_and_retrieve_input(
        ...
        if response.status() == hyper::StatusCode::ACCEPTED {
            info!("Response status is ACCEPTED, skipping");
            return Err("Skip".into());
        }
        info!("Response status is not ACCEPTED, retrieving input");
        let result = RollupInput::try_from_async(response).await?;

        Ok(result)
    }

    pub async fn send_report(report: Value) -> Result<Response<Body>, Box<dyn std::error::Error>> {
