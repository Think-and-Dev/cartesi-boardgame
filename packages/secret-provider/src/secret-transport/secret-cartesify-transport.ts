 ...
 */
export class SecretCartesifyTransport extends SecretTransport {
  /**
  ...
  constructor(opts: CartesifyTransportOpts) {
    ...
    this.cartesifyFetch = Cartesify.createFetch({
      ...
      provider: opts.signer?.provider || undefined,
      signer: opts.signer,
    });
  }
   ...
   * @param hashes - Array of card hashes with their keys
   */
  async sendShuffledDeck(
    hashes: Array<{ key: string; hash: string }>
  ): Promise<void> {
    await this.cartesifyFetch(`${this.url}/advance`, {
      method: "POST",
      body: JSON.stringify({
        type: "STORE_SHUFFLED_DECK",
        payload: { hashes, matchID: this.matchID },
      }),
    });
  }
