pub mod routes {
    ...
    async fn consume_buffer(
        ...
        info!(logger, "Received finish request from DApp {:?}", body);
        ...
        debug!(logger, "No input found on buffer, sending finish request");
        let rollup_input = match send_finish_and_retrieve_input("accept").await {
            Ok(input) => input,
            Err(e) => {
                if e.to_string() == "Skip" {
                    debug!(logger, "Skip sending finish request, waiting for next input");
                    return HttpResponse::Accepted().finish();
                }
                else {
                    debug!(logger, "Error sending finish request, skipping");
    ...
    async fn request_random(
        ...
        // loop until we have the randomness. The idea is to block the random request from the DApp until we have the randomness
        loop {
            let randomness: Option<String> = ctx.get_randomness_for_timestamp(query.timestamp);
            if let Some(randomness) = randomness {
                debug!(logger, "Randomness already found: {:?}", randomness);
            ...
            match rollup_input.request_type.as_str() {
                "advance_state" => {
                    ...
                    match get_drand_beacon(&rollup_input.data.payload) {
                        ...
                        Err(e) => {
                            if e.to_string() == "Not a drand beacon" {
                                info!(logger, "Not a drand beacon, adding to buffer");
